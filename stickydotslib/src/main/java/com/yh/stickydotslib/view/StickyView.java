package com.yh.stickydotslib.view;
...
import com.yh.stickydotslib.utils.DisplayUtils;
import com.yh.stickydotslib.utils.GeometryUtil;
...
public class StickyView extends View {
     ...
     * 固定圆的切点
     */
    private PointF[] mFixTangentPoints = new PointF[]{new PointF(100, 190),
            new PointF(100, 210)};

    /**
     * 拖拽圆的切点
     */
    private PointF[] mDragTangentPoints = new PointF[]{new PointF(200, 180),
            new PointF(200, 220)};
    /**
     * 最大拖拽范围
     */
    ...
    private void init() {
        ...
        mParams.gravity = Gravity.TOP | Gravity.LEFT; // 从左上角添加控件
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (!isOut) {
            ...
            // 接下来是计算两个圆的外切点，会用到一点几何知识
            float dy = mDragCanterPoint.y - mFixCanterPoint.y;
            float dx = mDragCanterPoint.x - mFixCanterPoint.x;

            if (dx != 0) {
                float k1 = dy / dx; // 两圆心直线的斜率（与x轴夹角的tan值）
                float k2 = -1 / k1; // 与圆心垂直直线的斜率
                // 获得拖拽圆的两个切线点
                mDragTangentPoints = GeometryUtil.getIntersectionPoints(
                        mDragCanterPoint, mDragRadius, (double) k2);
                // 获得固定圆的两个切线点
                mFixTangentPoints = GeometryUtil.getIntersectionPoints(
            ...
            // quadTo是绘制二阶贝塞尔曲线，二阶的话需要一个控制点，一个起点一个终点
...
//        当在范围外松手的时候是不再绘制拖拽圆的
    ...
    private float updateStickRadius() {
        ...
        // distance在0-1之前，固定圆的半径在固定半径到最小之间
